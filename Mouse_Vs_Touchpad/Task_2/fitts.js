...
FittsLawExperiment.prototype = {
	...
	//this.homingTarget.observe('mousedown', this.clickHandler.bindAsEventListener(this));
	//this.clickTarget.observe('mousedown', this.clickHandler.bindAsEventListener(this));
	document.body.observe('click', this.clickHandler.bindAsEventListener(this));
	//document.body.observe('click', this.homingHandler.bindAsEventListener(this));
	...
	if (this.clickTarget == el) {
	    ...
	    this.clickTarget.className = 'active-target';
            ...
            this.createCircle(this.clickTarget, centerx + deltax, centery + deltay, radius, 'blue');
	} else {
	    this.homingTarget.className = 'active-target';
	    this.clickTarget.className = 'inactive-target';
            this.createCircle(this.homingTarget, centerx, centery, this.options.homingRadius, 'blue');
	...
	if (this.state == FittsLawExperiment.kStateRunning) {
			...
			if (clickCount % 2 == 1) {
				...
				if (insidetarget && e.target == this.homingTarget) {
					...
					if (this.homingTarget.className == 'active-target') {
						console.log('err');
